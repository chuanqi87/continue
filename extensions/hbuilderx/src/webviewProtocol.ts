  ...
  implements IMessenger<FromWebviewProtocol, ToWebviewProtocol>
{
  ...
  _webview?: any; //hx.Webview;
  _webviewListener?: any; //hx.Disposable;

  get webview(): any | undefined {
    return this._webview;
  }

  set webview(webView: any) {
  ...
  public request<T extends keyof ToWebviewProtocol>(
    ...
    console.log("[hbuilderx] webviewProtocol.request 开始", {
      messageType,
      hasData: !!data,
      retry,
    });
    ...
    return new Promise(async (resolve) => {
      if (retry) {
        ...
        while (!this.webview) {
          if (i >= 10) {
            console.error(
              "[hbuilderx] webviewProtocol.request webview未准备好，超时",
              { messageType },
            );
            resolve(undefined);
            return;
          } else {
            console.log("[hbuilderx] webviewProtocol.request 等待webview", {
              messageType,
              attempt: i,
            });
      ...
      console.log("[hbuilderx] webviewProtocol.request 发送消息", {
        messageType,
        messageId,
      });
      ...
      if (this.webview) {
        // TODO: 此处无disposable返回值，可能会存在bug
