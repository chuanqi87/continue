...
export class VerticalDiffHandler implements hx.Disposable {
  public insertedInCurrentBlock = 0;
  disposables: hx.Disposable[] = [];
  ...
  constructor(
    ...
    const disposable = hx.window.onDidChangeActiveTextEditor((editor: any) => {
      if (!editor) {
        return;
      }
  ...
  async queueDiffLine(diffLine: DiffLine | undefined) {
    ...
    if (this._queueLock || this.editor !== hx.window.activeTextEditor) {
      return;
    }
  ...
  async run(diffLineGenerator: AsyncGenerator<DiffLine>) {
    ...
    try {
      ...
      const range = new hx.Range(this.startLine, 0, this.startLine, 0);
      this.editor.revealRange(range, hx.TextEditorRevealType.Default);
  ...
  async reapplyWithMyersDiff(diffLines: DiffLine[]) {
    ...
    await this.editor.edit((editBuilder: any) => {
    ...
    await this.editor.edit(
      (editBuilder: any) => {
        ...
        if (index >= lineCount) {
          ...
          editBuilder.insert(new hx.Position(index, 0), `${text}\n`);
        }
      },
      {
        undoStopAfter: false,
  ...
  private async deleteLinesAt(index: number, numLines = 1) {
    const startLine = new hx.Position(index, 0);
    await this.editor.edit(
      (editBuilder: any) => {
        editBuilder.delete(
          new hx.Range(startLine, startLine.translate(numLines)),
        );
      },
      {
        undoStopAfter: false,
  ...
  private async deleteRangeLines(ranges: hx.Range[]) {
    await this.editor.edit(
      (editBuilder: any) => {
        for (const range of ranges) {
          editBuilder.delete(
            new hx.Range(range.start, new hx.Position(range.end.line + 1, 0)),
          );
        }
      },
      {
  ...
  private updateIndexLineDecorations() {
    ...
    if (this.currentLineIndex - this.newLinesAdded >= this.endLine) {
      ...
      const start = new hx.Position(this.currentLineIndex, 0);
      this.editor.setDecorations(indexDecorationType, [
        new hx.Range(
          start,
          new hx.Position(start.line, Number.MAX_SAFE_INTEGER),
        ),
      ]);
      const end = new hx.Position(this.endLine, 0);
      this.editor.setDecorations(belowIndexDecorationType, [
        new hx.Range(start.translate(1), end.translate(this.newLinesAdded)),
      ]);
    }
  }
