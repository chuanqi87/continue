...
const addedLineDecorationType = hx.window.createTextEditorDecorationType({
  isWholeLine: true,
...
export const indexDecorationType = hx.window.createTextEditorDecorationType({
  isWholeLine: true,
...
function translateRange(range: hx.Range, lineOffset: number): hx.Range {
  return new hx.Range(
...
export class AddedLineDecorationManager {
  ...
  applyToNewEditor(newEditor: any) {
...
export class RemovedLineDecorationManager {
  ...
  ranges: {
    line: string;
    range: hx.Range;
    decoration: hx.TextEditorDecorationType;
  }[] = [];
