...
class HbuilderXIde implements IDE {
  ...
  /**
   * 设置当前选中的工作区路径
   * 这会影响getWorkspaceDirs()的返回顺序，将选中的工作区放在第一位
   */
  setSelectedWorkspace(workspacePath: string): void {
    console.log("[hbuilderx] Setting selected workspace:", workspacePath);
  ...
  async getTerminalContents(): Promise<string> {
    // HBuilderX暂不支持终端内容读取API
    // 返回空字符串，不影响基本功能
    console.log("[hbuilderx] getTerminalContents 暂未实现HBuilderX终端API");
    return "";
  }
  ...
  getWorkspaceDirs(): Promise<string[]> {
    const allDirs = this.ideUtils
      .getWorkspaceDirectories()
      .map((uri: any) => uri.toString());

    // 如果用户选择了工作区，将其放在列表第一位
    if (this.selectedWorkspacePath) {
      const selectedIndex = allDirs.findIndex(
        (dir: string) => dir === this.selectedWorkspacePath,
      );
      if (selectedIndex > 0) {
        // 将选中的工作区移到第一位
        const [selected] = allDirs.splice(selectedIndex, 1);
        allDirs.unshift(selected);
        console.log(
          "[hbuilderx] Reordered workspace dirs, selected first:",
          allDirs,
        );
      }
    }
  ...
  async runCommand(
    ...
    console.log(
      "[hbuilderx] runCommand called, command:",
      command,
      "options:",
      options,
    );

    // HBuilderX暂不支持终端API，使用subprocess执行命令
    // 未来HBuilderX支持终端API后，可以参考VSCode的实现
    try {
      const cwd = options.cwd;
      await this.subprocess(command, cwd);
    } catch (error) {
      console.error("[hbuilderx] runCommand error:", error);
      throw error;
    }
  }
  ...
  async getProblems(fileUri?: string | undefined): Promise<Problem[]> {
    // HBuilderX暂不支持语言服务诊断API
    // 返回空数组，不影响基本功能
    console.log(
      "[hbuilderx] getProblems 暂未实现HBuilderX诊断API, fileUri:",
      fileUri,
    );
    return [];
  }
