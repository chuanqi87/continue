...
import { PromiseAdapter } from "./promiseUtils";
...
const controlPlaneEnv = getControlPlaneEnvSync(false ? "production" : "none");
...
export class WorkOsAuthProvider /* implements AuthenticationProvider, Disposable */ {
  ...
  private _codeExchangePromises = new Map<
    string,
    { promise: Promise<string>; cancel: any }
  >();
  ...
  constructor(
    ...
    // this._disposable = hx.Disposable.from(
    //   hx.authentication.registerAuthenticationProvider(
    //     controlPlaneEnv.AUTH_TYPE,
    //     AUTH_NAME,
    //     this,
    //     { supportsMultipleAccounts: false },
    //   ),
    //   hx.window.registerUriHandler(this._uriHandler),
    // );
  ...
  get ideRedirectUri() {
    ...
    url.pathname = `/auth/${hx.env.uriScheme}-redirect`;
    return url.toString();
  }
  ...
  public async createSession(
    ...
    try {
      ...
      hx.window.showErrorMessage(`Sign in failed: ${e}`);
      throw e;
    }
  }
  ...
  private async login(
    ...
    return await hx.window.withProgress(
      {
        location: hx.ProgressLocation.Notification,
        title: "Signing in to Continue...",
        cancellable: true,
      },
      async (_: any, token: any) => {
        ...
        const oauthUrl = url;
        if (oauthUrl) {
          await hx.env.openExternal(hx.Uri.parse(oauthUrl.toString()));
        } else {
          return;
        }
...
export async function getControlPlaneSessionInfo(
  ...
  try {
    ...
    // const session = await hx.authentication.getSession(
    //   controlPlaneEnv.AUTH_TYPE,
    //   [],
