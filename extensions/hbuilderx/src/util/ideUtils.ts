...
import { openEditorAndRevealRange } from "./hbuilderx";
// import {
//   SuggestionRanges,
...
import { getUniqueId } from "./hbuilderx";
...
export class HbuilderXIdeUtils {
  ...
  //   );
  // }
  private _workspaceDirectories: any[] | undefined = undefined;
  getWorkspaceDirectories(): any[] {
  ...
  showVirtualFile(name: string, contents: string) {
    // HBuilderX暂不支持虚拟文档scheme，使用untitled文档实现
    const doc = hx.workspace.openTextDocument({
      content: contents,
      language: name.endsWith(".md") ? "markdown" : undefined,
    });
    doc.then((document: any) => {
      hx.window.showTextDocument(document);
    });
  }
  ...
  async writeFile(filePath: string, contents: string): Promise<void> {
    ...
    try {
      ...
      const fsPath = uriToFsPath(filePath);

      // 使用 path.dirname 和 path.normalize 处理跨平台路径
      ...
      await fs.promises.writeFile(normalizedPath, contents, "utf8");
  ...
  //   );
  // }
  async openFile(uri: any, range?: any) {
    // vscode has a builtin open/get open files
    return await openEditorAndRevealRange(uri, range, hx.ViewColumn.One, false);
  }
  async fileExists(uri: any): Promise<boolean> {
    try {
  ...
  async readDirectory(
    ...
    return await this.fsOperation(
      uri,
      async (u) => {
        const dirPath = uriToFsPath(uri);
        const entries = await fsReaddir(dirPath, { withFileTypes: true });

        return entries.map((entry): [string, number] => {
          let type: number;
          if (entry.isFile()) {
            type = NodeFileType.File;
          } else if (entry.isDirectory()) {
            type = NodeFileType.Directory;
          } else if (entry.isSymbolicLink()) {
            type = NodeFileType.SymbolicLink;
          } else {
            type = NodeFileType.Unknown;
          }
  ...
  getOpenFiles(): any[] {
    return [];
    // return hx.window.tabGroups.all
    //   .map((group: any) => {
    //     return group.tabs.map((tab: any) => {
    //       return (tab.input as any)?.uri;
    //     });
    //   })
    //   .flat()
    //   .filter(Boolean) // filter out undefined values
    //   .filter((uri: any) => this.documentIsCode(uri)); // Filter out undesired documents
  }
  saveFile(uri: any) {
  ...
  //   }
  // }
  private async _getRepo(forDirectory: any): Promise<Repository | undefined> {
    // Use the native git extension to get the branch name
    const extension = hx.extensions.getExtension("vscode.git");
    if (
      typeof extension === "undefined" ||
      !extension.isActive ||
      typeof hx.workspace.workspaceFolders === "undefined"
    ) {
      return undefined;
    }
    try {
  ...
  private _getRepositories(): Repository[] | undefined {
    ...
    try {
      ...
      this._repoWasNone = true;
      console.warn("[hbuilderx] Git not found: ", e);
      return undefined;
    }
  }
  ...
  async getRepo(forDirectory: any): Promise<Repository | undefined> {
    ...
    while (!repo?.state?.HEAD?.name) {
      ...
      i++;
      if (i >= HbuilderXIdeUtils.secondsToWaitForGitToLoad) {
        this._repoWasNone = true;
        return undefined;
      }
  ...
  async getGitRoot(forDirectory: any): Promise<any | undefined> {
    const repo = await this.getRepo(forDirectory);
    return repo?.rootUri;
  }
  async getBranch(forDirectory: any) {
    const repo = await this.getRepo(forDirectory);
  ...
  async getDiff(includeUnstaged: boolean): Promise<string[]> {
    ...
    try {
      ...
      console.error("[hbuilderx] getDiff error:", e);
      return [];
    }
  }
}
