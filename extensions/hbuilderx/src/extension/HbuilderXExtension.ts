...
import { HarmonyConfigsManager } from "../util/harmonyConfigsManager";
...
export class HbuilderXExtension {
  ...
  constructor(extensionContext: any) {
    ...
    // 初始化完成后，检测并管理Harmony配置目录（harmony-configs 和 harmony-mp-configs）
    console.log("[hbuilderx] 初始化Harmony配置管理器");
    const harmonyConfigsManager = new HarmonyConfigsManager(this.ide);
    harmonyConfigsManager.checkAndUpdateAll().catch((error) => {
      console.error("[hbuilderx] Harmony配置管理器执行失败:", error);
    });
  }
