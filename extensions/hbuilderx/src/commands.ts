...
import { ContextMenuConfig, ILLM } from "core";
...
// 添加缺失的类型定义
interface QuickEditShowParams {
  range?: any;
  prompt?: string;
  [key: string]: any;
}

let fullScreenPanel: any | undefined;
...
function hideGUI() {
  ...
  if (fullScreenTab) {
    ...
    // focus sidebar
    hx.commands.executeCommand("workbench.action.closeAuxiliaryBar");
    // hx.commands.executeCommand("workbench.action.toggleAuxiliaryBar");
  }
}
...
const getCommandsMap: (
  ...
  return {
    ...
    "continue.quickFix": async (range: any, diagnosticMessage: string) => {
    ...
    "continue.defaultQuickAction": async (args: QuickEditShowParams) => {
      ...
      hx.commands.executeCommand("continue.focusEdit", args);
    },
    ...
    "continue.clearConsole": async () => {
      console.warn("[hbuilderx] clearConsole 消息未实现");
    ...
    "continue.focusContinueSessionId": async (
      ...
      if (!sessionId) {
        sessionId = await hx.window.showInputBox({
    ...
    "continue.toggleFullScreen": async () => {
      ...
      let panel = hx.window.createWebviewPanel(
      ...
      //TODO: 这里需要修改, 可能会有bug
      panel.webview.html = await sidebar.getSidebarContent(
        extensionContext,
        panel,
        undefined,
        undefined,
      ...
      hx.commands.executeCommand("workbench.action.copyEditorToNewWindow");
      hx.commands.executeCommand("workbench.action.closeAuxiliaryBar");
    },
    ...
    "continue.openTabAutocompleteConfigMenu": async () => {
      ...
      // const config = hx.workspace.getConfiguration(EXTENSION_NAME);
      // const quickPick = hx.window.createQuickPick();
      ...
      //     hx.commands.executeCommand("continue.giveAutocompleteFeedback");
      ...
      //     hx.commands.executeCommand("continue.navigateTo", "/config");
      //   }
      //   quickPick.dispose();
      // });
      // quickPick.show();
    ...
    "continue.giveAutocompleteFeedback": async () => {
      const feedback = await hx.window.showInputBox({
        ignoreFocusOut: true,
        prompt:
      ...
      hx.window
        ...
        .then((selection: string | undefined) => {
          if (selection === "Read the docs") {
            hx.env.openExternal(
              hx.Uri.parse("https://docs.continue.dev/yaml-migration"),
            );
          }
        });
    },
...
const registerCopyBufferService = (context: any, core: Core) => {
  const typeDisposable = hx.commands.registerCommand(
    "editor.action.clipboardCopyAction",
    async (arg: any) => doCopy(typeDisposable),
  );

  async function doCopy(typeDisposable: any) {
    typeDisposable.dispose(); // must dispose to avoid endless loops

    await hx.commands.executeCommand("editor.action.clipboardCopyAction");

    const clipboardText = await hx.env.clipboard.readText();
    ...
    typeDisposable = hx.commands.registerCommand(
      "editor.action.clipboardCopyAction",
...
export function registerAllCommands(
  ...
  for (const [command, callback] of Object.entries(
    ...
    context.subscriptions.push(hx.commands.registerCommand(command, callback));
  }
